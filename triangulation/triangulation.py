...
## updated 02/11 - stable triangulation + 3D visualization + RMS summary
...
# ---------- Camera utilities ----------
def euler_to_rot_autel(yaw_deg, pitch_deg, roll_deg, yaw_sign=-1.0, apply_ned2enu=True, transpose_R=False):
...
# ---------- Build initial tracks from matches (fixed) ----------
...
for mf in tqdm(match_files, desc="Building tracks"):
    ...
    # Use a per-image unique ID for every keypoint (not shared idx)
    for (u1, v1), (u2, v2) in zip(pts1, pts2):
        key1 = (img1, float(u1), float(v1))
        key2 = (img2, float(u2), float(v2))
        ...
        if t1 is not None and t2 is not None:
            ...
            track_id = t1
            track_obs[track_id].append((img2, (u2,v2)))
            point_to_track[key2] = track_id
        elif t2 is not None:
            track_id = t2
            track_obs[track_id].append((img1, (u1,v1)))
            point_to_track[key1] = track_id
        else:
            track_id = next_track_id
            track_obs[track_id] = [(img1, (u1,v1)), (img2, (u2,v2))]
            point_to_track[key1] = track_id
            point_to_track[key2] = track_id
            next_track_id += 1
...
C_all = np.array([cam["C"] for cam in camera_poses.values()])
X_all = np.array([p[0] for p in filtered_points])

ax.scatter(X_all[:,0], X_all[:,1], X_all[:,2], s=3, c='cyan', label='Triangulated Points')
ax.scatter(C_all[:,0], C_all[:,1], C_all[:,2], c='red', s=40, label='Camera Centers')
for i, (img, cam) in enumerate(camera_poses.items()):
    ax.text(cam["C"][0], cam["C"][1], cam["C"][2], img[-8:-4], fontsize=6)
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.legend()
...
# ---------- RMS summary ----------
rms = np.sqrt(np.mean(np.square(reproj_errors)))
print(f"\nGlobal RMS reprojection error: {rms:.2f}px")

print(f"\nSaved triangulated points and camera poses to {OUTPUT_DIR}")
